...
export default async function CleanerCompanyOrders() {
  ...
  const { data: cleaner } = await admin.from('cleaner_profiles').select('verification_status,is_available,work_zone').eq('user_id', user.id).maybeSingle();
  if (!cleaner) redirect('/profile');
  const { data: ownWork } = await admin.from('order_workers').select('order_id').eq('cleaner_id', user.id);
  const ownOrderIds = (ownWork ?? []).map(item => item.order_id);
  const { data: communityMember } = await admin.from('community_cleaners').select('community_id,cleaner_communities(name)').eq('cleaner_id', user.id).maybeSingle();
  ...
  const orders = (loadedOrders ?? []).filter(item => ownOrderIds.includes(item.id) || (item.status === 'accepted' && item.selected_company_id && allowedCompanyIds.has(item.selected_company_id)));
