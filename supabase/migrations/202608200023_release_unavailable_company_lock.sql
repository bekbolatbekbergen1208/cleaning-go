-- A client must not be prevented from ordering because a formerly linked company
-- was later suspended, rejected, or removed from the approved-company catalogue.
update public.client_profiles cp
set company_locked = false
where cp.company_locked
  and not exists (
    select 1
    from public.company_profiles company
    where company.id = cp.preferred_company_id
...
begin
  ...
  if locked and exists(select 1 from company_profiles where id=preferred_company and verification_status='approved') then
    chosen_company:=preferred_company;
  else
    if locked then update client_profiles set company_locked=false where user_id=uid; end if;
